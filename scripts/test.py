...
def batchify(byte_chunks: List[np.ndarray], masks: List[np.ndarray], batch_size: int):
    ...
    for i in range(0, len(byte_chunks), batch_size):
        ...
        if len(batch_byte_chunks) < batch_size:
            ...
            batch_byte_chunks += [np.zeros_like(batch_byte_chunks[0])] * pad_size
            batch_masks += [np.zeros_like(batch_masks[0])] * pad_size
...
def process_file(arg):
    ...
    if file.is_file():
        ...
        output_path = pathlib.Path(output) / (str(rel_path) + ".json")
        # output_path = pathlib.Path(output) / rel_path
        # cmd = f"{args.client} --host {args.host} -P {args.port} -c grpc -f {file} -t {args.thread} -k {args.tokenizer} -b {args.batch_size}  -T {args.max_token} -w {args.window_size} -q {args.seq_len} -o {output_path} -m {model}" + \
        #     (" --prune" if args.prune else "")
        ...
        for sequence, mask in zip(batched_byte_chunks, batched_masks):
            ...
            result = send_request(stub, model, sequence, is_64_bit)
            probs.append(result[mask])
        probs = np.concatenate(probs, axis=0).flatten()
        offsets = np.arange(len(probs))
        instructions = offsets[probs > args.threshold] + base_addr
